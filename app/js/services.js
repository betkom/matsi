...
.factory('FellowService', ['$firebase', '$cookies', '$stateParams', '$rootScope', function($firebase, $cookies, $stateParams, $rootScope) {
        ...
        return {
            ...
            regRequest: function(uid) {
                rootRef.child('users').child($rootScope.currentUser.uid).child('sentRequests').child(uid).push({
                    timestamp: Firebase.ServerValue.TIMESTAMP
                });
                rootRef.child('users').child(uid).child('requests').child($rootScope.currentUser.uid).push({
                    timestamp: Firebase.ServerValue.TIMESTAMP
                });
            },
            acceptRequest: function(mentor) {
                ...
                rootRef.child('users').child($rootScope.currentUser.uid).set({
                    'isMentored': 'true'
                });
                rootRef.child('users').child($rootScope.currentUser.uid).child('mentors').child(mentor).set({
                    timestamp: Firebase.ServerValue.TIMESTAMP
                });
